...
# negative edges represent edges which don't exist in the graph and are labelled as 0
def generate_neg_edges(G, num_neg_edges):
    ...
    neg_edges = random.sample(list(G_comp.edges()), num_neg_edges)
    return neg_edges
...
def train_test_split(train_positive_edges, train_negative_edges, test_positive_edges,
    ...
    """
    creates X_train_, y_train, X_test, y_test for classification task.
    """
    X_train = []
    y_train = []
    ...
    print("Training and testing data is created!")
    return X_train, y_train, X_test, y_test


def prediction(classifier, X_train, y_train, X_test, y_test):
    """
    computes the predictions of test edges
    """
...
def main(original_edges_path, node_embeddings_path, training_edges_path, test_edges_path):
    """
    computes the link prediction accuracy, given an operator,
...
if __name__ == "__main__":
    ...
    main(args.original_edges_path, args.node_embeddings_path, args.training_edges_path, args.test_edges_path)